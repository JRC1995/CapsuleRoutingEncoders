...
from models.modules.heinsen_routing import Routing
...
class Classifier(nn.Module):
    ...
    def __init__(self, embeddings, pad_idx, classes_num,
        ...
        self.a_score = nn.Linear(self.D, 1)
        self.compress = nn.Linear(self.ks*self.output_channel, self.D)

        self.capsulize = Routing(d_cov=1, d_inp=self.D, d_out=self.D, n_out=self.classes_num+1)
    ...
    def forward(self, x, mask):
        ...
        x = self.compress(x)
        inp_scores = T.sigmoid(self.a_score(x)).view(N, S)*mask.view(N, S)
        _, class_embs, _ = self.capsulize(inp_scores,
                                          x.view(N, S, 1, self.D))

        class_embs = class_embs.view(N, self.classes_num+1, self.D)
        class_embs = class_embs[:, 1:, :]  # remove orphan
        #class_embs = self.dropout2(class_embs)

        # print(class_embs.size())

        logit = T.sigmoid(self.classifier(class_embs).view(N, self.classes_num))
